...
def validation_loop(sess, g, n_batches, chars=None, val_gen = None, tb_writer=None):
  ...
  for i in range(n_batches):
    ...
    # -- Autoregression loop is built into the beam search graph
      ...
      print("x, y: ", x.shape, y.shape)
      print("g.enc:", g.enc.shape)
      enc = sess.run( g.enc, feed_dict)
      
      # Saving the feature vector
      print("\nFile:",file_name, enc.shape)
      # if not os.path.exists(os.path.dirname("results/"+file_name)):
      #     os.makedirs(os.path.dirname("results/"+file_name))
      np.save(file_name,enc)
